...
- All your project code into a single main package (`automl-llm/`)
...
```bash
<root directory>
├── automl-llm/             # main package (should be named after your project)
...
1. Use this template to create a new GitHub repository, say `automl-llm`
...
2. Clone the `automl-llm` repository

3. Open a shell into your local `automl-llm` directory and run
    ```bash
    ./rename-template.sh automl-llm
    ```
    
    This will coherently rename the template's project name with the one chosen by you (i.e. `automl-llm`, in this example)
...
This will execute the `__main__.py` file in the `automl-llm` package:
```bash
poetry run python -m automl-llm
```
